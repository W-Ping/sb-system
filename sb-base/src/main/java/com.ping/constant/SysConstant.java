package com.ping.constant;

/**
 * @author lwp
 * @create 2019/07/14 10:47
 */
public class SysConstant {
	public static final String USER_INFO_SESSION_KEY = "user-info-session";
	public static final String PATH_HOUSE = "info";
	public static final String PATH_BUDGET = "budget";
	public static final String PATH_DECORATION = "decoration";
	public static final String UNIQUEID_BUDGET_PRIFIX = "BG";
	public static final String UNIQUEID_HOUSE_PRIFIX = "HS";
	public static final String UNIQUEID_HOUSE_DETAIL_PRIFIX = "HSD";
	public static final int STATUS_0 = 0;
	public static final int STATUS_1 = 1;
	public static final String SUCCESS_CODE = "success";
	public static final String FAIL_CODE = "fail";
}
