...
public class HouseViewController {
     ...
     */
    @GetMapping("/index")
    public String toIndex(HttpSession session, @RequestParam("mobile_phone") String mobilePhone) {
        UserInfoVo userInfoVo = (UserInfoVo) session.getAttribute(SysConstant.USER_INFO_SESSION_KEY);
        if (userInfoVo == null) {
            return "redirect:/login/index";
        }
        if (SysConstant.ADMIN_POST_TYPE.equals(userInfoVo.getLoginPostType())) {
            return "redirect:/house/page/admin?mobile_phone=" + mobilePhone;
        }
        return "redirect:/house/page/0?mobile_phone=" + mobilePhone;
    }
     ...
     */
    @GetMapping("/admin")
    public String toAdmin(Model model, HttpSession session, @RequestParam("mobile_phone") String mobilePhone) {
        model.addAttribute("mobilePhone", mobilePhone);
        UserInfoVo userInfoVo = (UserInfoVo) session.getAttribute(SysConstant.USER_INFO_SESSION_KEY);
        if (userInfoVo == null || !SysConstant.ADMIN_POST_TYPE.equals(userInfoVo.getLoginPostType())) {
            return "redirect:/login/index";
        }
    ...
    public String index(Model model, @PathVariable("path") int path, @RequestParam(name = "mobile_phone") String mobilePhone) {
        ...
        Map<String, BigDecimal> totalAmountMap = null;
