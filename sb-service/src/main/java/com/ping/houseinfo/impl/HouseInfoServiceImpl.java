...
public class HouseInfoServiceImpl extends BaseService implements IHouseInfoService {
	 ...
	 */
	@Override
	public Map<String, BigDecimal> calculateBudgetTotalAmount(final String mobilePhone) {
		Map<String, BigDecimal> result = new HashMap<>();
		BigDecimal maxValue = BigDecimal.ZERO;
		BigDecimal minValue = BigDecimal.ZERO;
		BigDecimal selfValue = BigDecimal.ZERO;
		List<HouseBudgetInfoPo> maxList = iHouseBudgetInfoMapper.queryBudgetAmountDetailOfMax(mobilePhone);
		if (!CollectionUtils.isEmpty(maxList)) {
			maxValue = maxList.stream().map(v -> v.getBudgetAmount().multiply(new BigDecimal(v.getBudgetCount())).setScale(2, BigDecimal.ROUND_HALF_UP))
					.reduce(BigDecimal.ZERO, BigDecimal::add);
		}
		List<HouseBudgetInfoPo> minList = iHouseBudgetInfoMapper.queryBudgetAmountDetailOfMin(mobilePhone);
		if (!CollectionUtils.isEmpty(minList)) {
			minValue = minList.stream().map(v -> v.getBudgetAmount().multiply(new BigDecimal(v.getBudgetCount())).setScale(2, BigDecimal.ROUND_HALF_UP))
					.reduce(BigDecimal.ZERO, BigDecimal::add);
		}
		List<HouseBudgetInfoPo> selfList = iHouseBudgetInfoMapper.queryBudgetAmountDetailOfSelf(mobilePhone);
		if (!CollectionUtils.isEmpty(selfList)) {
			selfValue = maxList.stream().map(v -> v.getBudgetAmount().multiply(new BigDecimal(v.getBudgetCount())).setScale(2, BigDecimal.ROUND_HALF_UP))
					.reduce(BigDecimal.ZERO, BigDecimal::add);
		}
		result.put("max_account_total", maxValue);
		result.put("min_account_total", minValue);
		result.put("account_total", selfValue);
		return result;
	}
